...
class AddonController extends Controller {
	...
	public function __construct() {
		...
		$this->title 	 = 'Addon Service';
		$this->subtitle  = 'Addon Service';
		$this->heading   = 'Addon Service';
		$this->language  = 1;
	}
	public function index(Request $request){		
		...
		$data['dataCount'] 	= Addon::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
		$data['dataGrid']  	= Addon::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, $fieldname, $orderby, $limit, $offset);
		$data['pagenation'] = Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
		$data['title'] 		= $this->title;
		$data['heading'] 	= $this->heading;
		$data['module_ajaxurl']	= url('/').'/admin/addon_ajax/';
		$data['orderby']	= 'asc';
		$data['fa_order']	= 'asc';
		...
		$data['maxorder']   = Addon::maxOrderId($this->tablename);	
		$data['minorder']   = Addon::minOrderId($this->tablename);	
		$data['keywords'] 	= $keywords;
		$data['limit'] 		= $limit;
		return view('admin.addon.index',['data' => $data]);
	}
		...
		switch($type){
			case 'searching':$keywords 	= Input::all();	
							 ...
							 $request->session()->put('sessionData', $sessionArray);
							  /* SESSION VALUE */							 
							 $data['dataCount'] 	= Addon::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
							 $data['dataGrid']  	= Addon::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, 't.order_id', 'asc', $limit, 0);
							 ...
							 $data['insert_id'] = '';		
							 $data['maxorder']  = Addon::maxOrderId($this->tablename);	
							 $data['minorder']  = Addon::minOrderId($this->tablename);	 							 						 $data['keywords'] 	= $keywords;
							 $data['limit'] 	= $limit;
							 return view('admin.addon.listview',['data' => $data]);
							 break;
			...
			case 'ordering': $orderby 	= Input::get('orderby');
							 ...
							 $data['dataCount'] 	= Addon::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language), $keywords);
							 $data['dataGrid']  	= Addon::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language), $keywords, $fieldname, $orderby, $limit, 0);
							 $data['pagenation'] 	= Pagenation::getGroupPagination($data['dataCount'],1, $limit);
							 $data['title'] 		= $this->title;
							 $data['heading'] 		= $this->heading;
							 $data['module_ajaxurl']= url('/').'/admin/addon_ajax/';
			...
			case 'setactive':$sessionGet = array();
							 ...
							 $request->session()->put('sessionData', $sessionArray);							 
							 /* SESSION VALUE */							 						 
							 $update = Addon::updateData($this->tablename, $data=array('is_active'=>$newactive,'updated_at'=>date('Y-m-d H:i:s')), $where=array('id'=>$primary_id));
							 $data['dataCount'] = Addon::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
							 $data['dataGrid']  = Addon::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, $fieldname, $orderby, $limit, $offset);
							 $data['pagenation']= Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
			...
			case 'orderup':  $primary_id = Input::get('dataid');
							 ...
							 $orderup = Addon::orderUp($this->tablename,$primary_id, $current_order_id);
							 ...
							 $request->session()->put('sessionData', $sessionArray);
							 $data['dataCount'] 	= Addon::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords=array());
							 $data['dataGrid']  	= Addon::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords=array(), $fieldname, $orderby, $limit, $offset);
							 $data['pagenation'] = Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
							 $data['title'] 	= $this->title;
							 $data['heading'] 	= $this->heading;
							 $data['module_ajaxurl']	= url('/').'/admin/addon_ajax/';
			...
			case 'orderdown':$primary_id = Input::get('dataid');
							 $current_order_id = Input::get('noworder');
							 $orderup = Addon::orderDown($this->tablename,$primary_id, $current_order_id);	
							 /* SESSION VALUE */							 
							 $sessionGet = $request->session()->get('sessionData');
							 $orderby	 = !empty($sessionGet['orderby'])?$sessionGet['orderby']:'asc';
							 $fieldname  = !empty($sessionGet['fieldname'])?$sessionGet['fieldname']:'t.order_id';
			...
			case 'delete':	$sessionGet = array();
							 ...
							 $request->session()->put('sessionData', $sessionArray);							 
							 /* SESSION VALUE */								 						 
							 $delete = Addon::deleteData($this->tablename, $primary_id);
							 $data['dataCount'] = Addon::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
							 $data['dataGrid']  = Addon::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, $fieldname, $orderby, $limit, $offset);
							 $data['pagenation']= Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
	...
	public function update($id = '', Request $request) {
		...
		if($todo=='saverec'){	
				...
				$update = Addon::updateData($this->tablename, $data=array('is_active'=>$is_active,'updated_at'=>date('Y-m-d H:i:s')), $where=array('id'=>$id));								
				$update = Addon::updateData('addon_translation', $data=array('addon_name'=>$request->addon_name_1), $where=array('addon_id'=>$id,'language_id'=>1));				
				$update = Addon::updateData('addon_translation', $data=array('addon_name'=>$request->addon_name_2), $where=array('addon_id'=>$id,'language_id'=>2));				
		...
		$addon = DB::table('addon as acco')
			->select('*')
			->join('addon_translation as atrans', 'acco.id', '=', 'atrans.addon_id')
			->where('acco.id', $id)
			->get();		
		...
		$data['dataset'] 	=  $addon;
		$data['id'] 		=  $id;		
		return view('admin.addon.addon', ['data' => $data]);
	}
	...
	public function create(Request $request) {
		...
		if($todo=='addrec'){
			$is_active = (!isset($request->is_active)?0:1);
			$max_order = DB::table($this->tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_addon)"))->get();
			$addonId = DB::table($this->tablename)->insertGetId(
					['is_active' => $is_active, 'order_id' => $max_order[0]->order_id + 1, 'created_at' => date('Y-m-d H:i:s'), 'updated_at' => date('Y-m-d H:i:s')]
				);
			DB::table('addon_translation')->insert([
					['addon_id' => $addonId, 'language_id' => 1, 'addon_name' => $request->addon_name_1],
					['addon_id' => $addonId, 'language_id' => 2, 'addon_name' => $request->addon_name_2],
				]);
			$sessionInsert['insertid'] 	= $addonId;
