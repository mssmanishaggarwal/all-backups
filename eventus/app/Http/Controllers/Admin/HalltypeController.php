...
class HalltypeController extends Controller {
	...
	public function __construct() {
		...
		$this->title 	 = 'Hall Type';
		$this->subtitle  = 'Hall Type';
		$this->heading   = 'Hall Type';
		$this->language  = 1;
	}
	public function index(Request $request){		
		...
		$data['dataCount'] 	= Halltype::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
		$data['dataGrid']  	= Halltype::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, $fieldname, $orderby, $limit, $offset);
		$data['pagenation'] = Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
		$data['title'] 		= $this->title;
		$data['heading'] 	= $this->heading;
		$data['module_ajaxurl']	= url('/').'/admin/halltype_ajax/';
		$data['orderby']	= 'asc';
		$data['fa_order']	= 'asc';
		...
		$data['maxorder']   = Halltype::maxOrderId($this->tablename);	
		$data['minorder']   = Halltype::minOrderId($this->tablename);	
		$data['keywords'] 	= $keywords;
		$data['limit'] 		= $limit;
		return view('admin.halltype.index',['data' => $data]);
	}
		...
		switch($type){
			case 'searching':$keywords 	= Input::all();	
							 ...
							 $request->session()->put('sessionData', $sessionArray);
							  /* SESSION VALUE */							 
							 $data['dataCount'] 	= Halltype::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
							 $data['dataGrid']  	= Halltype::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, 't.order_id', 'asc', $limit, 0);
							 $data['pagenation'] = Pagenation::getGroupPagination($data['dataCount'],1,$limit);
							 $data['title'] 	= $this->title;
							 $data['heading'] 	= $this->heading;
							 $data['module_ajaxurl']	= url('/').'/admin/hall_type_ajax/';
							 $data['orderby']	= 'asc';
							 $data['fa_order']	= 'asc';
							 $data['hide_order']= 1;
							 $data['update_id'] = '';
							 $data['insert_id'] = '';	
							 $data['maxorder']  = Halltype::maxOrderId($this->tablename);	
							 $data['minorder']  = Halltype::minOrderId($this->tablename);	 							 						 $data['keywords'] 	= $keywords;
							 $data['limit'] 	= $limit;
							 return view('admin.halltype.listview',['data' => $data]);
							 break;
			...
			case 'ordering': $orderby 	= Input::get('orderby');
							 ...
							 $data['dataCount'] 	= Halltype::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language), $keywords);
							 $data['dataGrid']  	= Halltype::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language), $keywords, $fieldname, $orderby, $limit, 0);
							 $data['pagenation'] 	= Pagenation::getGroupPagination($data['dataCount'],1, $limit);
							 $data['title'] 		= $this->title;
							 $data['heading'] 		= $this->heading;
							 $data['module_ajaxurl']= url('/').'/admin/hall_type_ajax/';
			...
			case 'setactive':$sessionGet = array();
							 ...
							 $request->session()->put('sessionData', $sessionArray);							 
							 /* SESSION VALUE */							 						 
							 $update = Halltype::updateData($this->tablename, $data=array('is_active'=>$newactive,'updated_at'=>date('Y-m-d H:i:s')), $where=array('id'=>$primary_id));
							 $data['dataCount'] = Halltype::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
							 $data['dataGrid']  = Halltype::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, $fieldname, $orderby, $limit, $offset);
							 $data['pagenation']= Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
			...
			case 'orderup':  $primary_id = Input::get('dataid');
							 ...
							 $orderup = Halltype::orderUp($this->tablename,$primary_id, $current_order_id);
							 ...
							 $request->session()->put('sessionData', $sessionArray);
							 $data['dataCount'] 	= Halltype::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords=array());
							 $data['dataGrid']  	= Halltype::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords=array(), $fieldname, $orderby, $limit, $offset);
							 $data['pagenation'] = Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
							 $data['title'] 	= $this->title;
							 $data['heading'] 	= $this->heading;
							 $data['module_ajaxurl']	= url('/').'/admin/hall_type_ajax/';
			...
			case 'orderdown':$primary_id = Input::get('dataid');
							 $current_order_id = Input::get('noworder');
							 $orderup = Halltype::orderDown($this->tablename,$primary_id, $current_order_id);	
							 /* SESSION VALUE */							 
							 $sessionGet = $request->session()->get('sessionData');
							 $orderby	 = !empty($sessionGet['orderby'])?$sessionGet['orderby']:'asc';
							 $fieldname  = !empty($sessionGet['fieldname'])?$sessionGet['fieldname']:'t.order_id';
			...
			case 'delete':	$sessionGet = array();
							 ...
							 $request->session()->put('sessionData', $sessionArray);							 
							 /* SESSION VALUE */								 						 
							 $delete = Halltype::deleteData($this->tablename, $primary_id);
							 $data['dataCount'] = Halltype::totalRecord('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords);
							 $data['dataGrid']  = Halltype::totalGrid('*', $this->tablename, $where=array('atrans.language_id'=>$this->language),$keywords, $fieldname, $orderby, $limit, $offset);
							 $data['pagenation']= Pagenation::getGroupPagination($data['dataCount'],$currpage, $limit);
	...
	public function update($id = '', Request $request) {
		...
		if($todo=='saverec'){	
				...
				$update = Halltype::updateData($this->tablename, $data=array('is_active'=>$is_active,'updated_at'=>date('Y-m-d H:i:s')), $where=array('id'=>$id));								
				$update = Halltype::updateData('hall_type_translation', $data=array('hall_type_name'=>$request->hall_type_name_1), $where=array('hall_type_id'=>$id,'language_id'=>1));				
				$update = Halltype::updateData('hall_type_translation', $data=array('hall_type_name'=>$request->hall_type_name_2), $where=array('hall_type_id'=>$id,'language_id'=>2));				
		...
		$accommodation = DB::table('hall_type as acco')
			->select('*')
			->join('hall_type_translation as atrans', 'acco.id', '=', 'atrans.hall_type_id')
			->where('acco.id', $id)
			->get();		
		...
		$data['id'] 		=  $id;		
		return view('admin.halltype.hall_type', ['data' => $data]);
	}
	...
	public function create(Request $request) {
		...
		$todo = Input::get('todo');
		if($todo=='addrec'){
			$max_order = DB::table($this->tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_hall_type)"))->get();
			
			
			$is_active = (!isset($request->is_active)?0:1);				
			$halltypeId = DB::table($this->tablename)->insertGetId(
					['is_active' => $is_active, 'order_id' => $max_order[0]->order_id + 1, 'created_at' => date('Y-m-d H:i:s'), 'updated_at' => date('Y-m-d H:i:s')]
				);
			DB::table('hall_type_translation')->insert([
					['hall_type_id' => $halltypeId, 'language_id' => 1, 'hall_type_name' => $request->hall_type_name_1],
					['hall_type_id' => $halltypeId, 'language_id' => 2, 'hall_type_name' => $request->hall_type_name_2],
				]);
			$sessionInsert['insertid'] 	= $halltypeId;
