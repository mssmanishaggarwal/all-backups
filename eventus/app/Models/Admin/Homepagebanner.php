...
class Homepagebanner extends Model
{
	...
	{		
		...
		$query->join('banner_translation as atrans', 't.id', '=', 'atrans.banner_id');
	...
	{
		$previous= DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_banner where order_id < " . $current_order_id . ")"))->get();
	...
	{
		$next= DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_banner where order_id > " . $current_order_id . ")"))->get();
	...
	public static function maxOrderId($tablename){
		$max_order = DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_banner)"))->get();
		return isset($max_order[0]->order_id)?$max_order[0]->order_id:0;
	}
	public static function minOrderId($tablename){
		$min_order = DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_banner)"))->get();
