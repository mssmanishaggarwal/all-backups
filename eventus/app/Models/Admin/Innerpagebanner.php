...
class Innerpagebanner extends Model
{
	...
	{		
		...
		if($keywords){
			foreach($keywords as $key=>$val){
				if($key=='inner_banner_title'){					
					$query->where($key, 'LIKE', '%'.$val.'%');
				}
			}			
		}	    
		$query->join('inner_banner_translation as atrans', 't.id', '=', 'atrans.inner_banner_id');
	    $totalRec = $query->count();
	    return $totalRec;
	}
	
	public static function getTableData($select, $tablename, $where) {
		return $innerpagebanner = DB::table($tablename)
			->select('*')
			->where($where)
			->get();
	}
	...
	{
		$previous= DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_inner_banner where order_id < " . $current_order_id . ")"))->get();
	...
	{
		$next= DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_inner_banner where order_id > " . $current_order_id . ")"))->get();
	...
	public static function maxOrderId($tablename){
		$max_order = DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_inner_banner)"))->get();
		return isset($max_order[0]->order_id)?$max_order[0]->order_id:0;
	}
	public static function minOrderId($tablename){
		$min_order = DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_inner_banner)"))->get();
