...
class Pricerange extends Model
{
	...
	{		
		...
		$query->join('price_range_translation as atrans', 't.id', '=', 'atrans.price_range_id');
		$query->join('currency as curr', 'curr.id', '=', 't.currency_id');
	...
	{
		$previous= DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_price_range where order_id < " . $current_order_id . ")"))->get();
	...
	{
		$next= DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_price_range where order_id > " . $current_order_id . ")"))->get();
	...
	public static function maxOrderId($tablename){
		$max_order = DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_price_range)"))->get();
		return isset($max_order[0]->order_id)?$max_order[0]->order_id:0;
	}
	public static function minOrderId($tablename){
		$min_order = DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_price_range)"))->get();
