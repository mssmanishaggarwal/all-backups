...
class Subscription extends Model
{
	...
	{		
		...
		if($keywords){
			foreach($keywords as $key=>$val){
				...
				if($key=='subscription_month'){					
					if($val!=''){
						$query->where($key, '=', $val);
					}
				}					
		}	    
		$query->join('subscription_translation as atrans', 't.id', '=', 'atrans.subscription_id');
	...
	{
		$previous= DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_subscription where order_id < " . $current_order_id . ")"))->get();
	...
	{
		$next= DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_subscription where order_id > " . $current_order_id . ")"))->get();
	...
	public static function maxOrderId($tablename){
		$max_order = DB::table($tablename)->where('order_id', DB::raw("(select max(`order_id`) from ev_subscription)"))->get();
		return isset($max_order[0]->order_id)?$max_order[0]->order_id:0;
	}
	public static function minOrderId($tablename){
		$min_order = DB::table($tablename)->where('order_id', DB::raw("(select min(`order_id`) from ev_subscription)"))->get();
