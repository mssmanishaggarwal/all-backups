...
<div class="box-header with-border">
               ...
               <div class="col-sm-4"> 
                    <input type='text' id='addon_name' class="form-control" placeholder="Addon Service Name" name="addon_name" value="{{isset($data['keywords']['addon_name'])?$data['keywords']['addon_name']:''}}" />
                    ...
                    <div class="col-sm-4"> 
					 ...
					 <a class="btn btn-default btn-flat" href="{{ url('/admin/addon_list') }}"><span class="fa fa-refresh fa-fw"></span> Reset</a>
                  </div>
                ...
                <div class="row">
                  ...
                  <a href="{{ url('/admin/addon') }}" class="btn-dark btn"> <span class="fa fa-plus fa-fw"></span> Add Addon Service</a>
        ...
        <div class="box box-default" id="recordset">			   
                <div class="box-body no-padding">
                    <table class="table table-striped table-hover">
						<tr class="warning">                                
							<th width="70%">
							<a href="javascript:;" onclick="ordering('ordering','{{$data['orderby']}}','atrans.addon_name')">ADDON SERVICE <i id="addon_name" class="fa fa-sort-alpha-{{$data['fa_order']}} text-muted"></i> </a></th>                       	<th width="15%" class="text-center">ORDER</th>
                            <th width="15%" class="text-right">ACTIONS</th>
                        </tr>
						@foreach($data['dataGrid'] as $dataset)
						<tr>
							<td>{{$dataset->addon_name}}
							@if($data['update_id']==$dataset->addon_id)
								<span class="label label-info">Updated</span>
							@endif
							@if($data['insert_id']==$dataset->addon_id)
							...
							@if($data['hide_order']== 0)
								@if($dataset->order_id!=$data['maxorder'])
									<a href="javascript:;" title="Down" onclick="resetorder('orderdown','{{$dataset->addon_id}}','{{$dataset->order_id}}')"><i class="fa fa-caret-down"></i></a>
								@endif
								@if($dataset->order_id!=$data['minorder']) 
									<a href="javascript:;" title="Up" onclick="resetorder('orderup','{{$dataset->addon_id}}','{{$dataset->order_id}}')"><i class="fa fa-caret-up"></i></a>
								@endif	
							@else
							--
							@endif
							...
							<button type="button" class="btn btn-default" onclick="setActive('setactive','{{$dataset->addon_id}}','{{$dataset->is_active}}')" title="Active"> 
							<span class="fa fa-check-square-o text-primary"></span></button>
							@else
							<button type="button" class="btn btn-default" onclick="setActive('setactive','{{$dataset->addon_id}}','{{$dataset->is_active}}')"> 
							<span class="fa fa-square-o  text-primary" title="Inactive"></span></button>
							@endif
							
							<button type="button" class="btn btn-default" onclick="document.location.href='{{ url('/admin/addon') }}/{{$dataset->addon_id}}'" title="Update"> <span class="fa fa-edit text-warning"></span></button>
  <button type="button" class="btn btn-default" title="Delete" onclick="deletedata('delete',{{$dataset->addon_id}})" ><span class="fa fa-trash text-danger"></span></button></div></td>
						</tr>
					    @endforeach
					</table>
