...
@section('content')
        <div id="main">
            <form id="advertisement_form" name="advertisement_form" method="POST" class="form-horizontal" enctype="multipart/form-data" action="{{$data['url']}}">
            ...
            <div class="box-header with-border">
               ...
               <label class="col-sm-2 text-right">{{$data['language'][$i]->lang_name}} Name <span class="text-red">*</span></label>
                <div class="col-sm-4"> <input type="text" class="form-control" name="advertisement_title_{{$data['language'][$i]->id}}" id="advertisement_title_{{$data['language'][$i]->id}}" value="{{ isset($data['dataset'][$i]->advertisement_title)?$data['dataset'][$i]->advertisement_title:''}}" required="required"/></div>
                </div> 					            
                    @endfor
                    ...
                    <div class="form-group">
                       ...
                       <div class="col-sm-10">
					   		@if(isset($data['dataset'][0]->advertisement_image))
					   	{{Html::image('public/uploads/advertisement/'.$data['dataset'][0]->advertisement_image, $data['dataset'][0]->advertisement_image)}}
					  @endif	
						   <input type="file" name="advertisement_image" id="advertisement_image" value="{{isset($data['dataset'][0]->advertisement_image)?$data['dataset'][0]->advertisement_image:''}}" @if($data['todo']=='addrec') required="required" @endif/>
					...
					<div class="form-group">
		               <label class="col-sm-2 text-right"> Advertisement Link <span class="text-red">*</span></label>
                       <div class="col-sm-4">
					   		<input type="text" class="form-control" id="advertisement_link" name="advertisement_link" value="{{isset($data['dataset'][0]->advertisement_link)?$data['dataset'][0]->advertisement_link:''}}" />
					   </div>
					  </div>
					<div class="form-group">
           ...
           <div class="box-footer text-right">
							...
							<button type="submit" class="btn btn-primary add_more_button add_contact" onclick="updateAdvertisement('{{ url('/admin/advertisement')}}/{{ $data['id']}}','advertisement_form','{{ url('/admin/advertisement_list')}}')"><span class="fa fa-save fa-fw"></span>{{ $data['saveBtn'] }}</button>
							@else
							<button type="submit" class="btn btn-primary add_more_button add_contact" onclick="updateAdvertisement('{{ url('/admin/advertisement')}}/{{ $data['id']}}','advertisement_form','{{ url('/admin/advertisement_list')}}')"><span class="fa fa-save fa-fw"></span>{{ $data['saveBtn'] }}</button>
							@endif
							
                            <button type="button" onclick="document.location.href='{{ url('/admin/advertisement_list') }}'" class="btn btn-default "><span class="fa fa-rotate-left fa-fw"></span> Cancel</button>
                  	</div>
					</div>
            </form>            
        </div>
