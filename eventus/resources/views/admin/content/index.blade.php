...
<div class="box-header with-border">
               ...
               <div class="col-sm-4"> 
                    <input type='text' id='cms_title' class="form-control" placeholder="Content Name" name="cms_title" value="{{isset($data['keywords']['cms_title'])?$data['keywords']['cms_title']:''}}" />
                    ...
                    <div class="col-sm-3"> 
					 ...
					 <a class="btn btn-default btn-flat" href="{{ url('/admin/content_list') }}"><span class="fa fa-refresh fa-fw"></span> Reset</a>
                  </div>
                ...
                <!--  <a href="{{ url('/admin/content') }}" class="btn-dark btn"> <span class="fa fa-plus fa-fw"></span> Add Content</a>-->
        ...
        <div class="box box-default" id="recordset">			   
                <div class="box-body no-padding">
                    <table class="table table-striped table-hover">
						<tr class="warning">                                
							<th width="55%"><a href="javascript:;" onclick="ordering('ordering','{{$data['orderby']}}','atrans.cms_title')">CONTENT <i id="cms_title" class="fa fa-sort-alpha-{{$data['fa_order']}} text-muted"></i> </a></th> 
						...
						<tr>
							<td>{{$dataset->cms_title}}
							@if($data['update_id']==$dataset->cms_id)
								<span class="label label-info">Updated</span>
							@endif
							@if($data['insert_id']==$dataset->cms_id)
								<span class="label label-success">New</span>
							@endif							
							</td>
							<td>{{($dataset->content_type==1)?'Page Type':'Block Type'}}</td>
							<td class="text-center">
							
							@if($data['hide_order']== 0)
								@if($dataset->order_id!=$data['maxorder'])
									<a href="javascript:;" title="Down" onclick="resetorder('orderdown','{{$dataset->cms_id}}','{{$dataset->order_id}}')"><i class="fa fa-caret-down"></i></a>
								@endif
								@if($dataset->order_id!=$data['minorder']) 
									<a href="javascript:;" title="Up" onclick="resetorder('orderup','{{$dataset->cms_id}}','{{$dataset->order_id}}')"><i class="fa fa-caret-up"></i></a>
								@endif	
							@else
							--
							@endif
							...
							<button type="button" class="btn btn-default" onclick="document.location.href='{{ url('/admin/content') }}/{{$dataset->cms_id}}'" title="Update"> <span class="fa fa-edit text-warning"></span></button>
