...
<section class="dash-main clearfix">
	...
	<div class="col-md-12 dash-container p-t-20 p-b-20 hallformwrap">		
		<ul class="nav nav-tabs custom-tab" role="tablist">
			<li role="presentation" class="active"><a href="{{url('/dashboard/my-booking')}}" aria-controls="home" role="tab" data-toggle="tab"><span class="fa fa-list-alt fa-fw"></span> {{ Sitevariable::setVariables($data['language_val'],'eventus_103')}}</a></li>
			<li role="presentation"><a href="{{url('/dashboard/booking-on-my-hall')}}" class="disable_link"><span class="fa fa-bookmark fa-fw"></span> {{ Sitevariable::setVariables($data['language_val'],'eventus_102')}}</a></li>

		</ul>
<div class="table-responsive">
	<h3>{{ Sitevariable::setVariables($data['language_val'],'eventus_103')}}</h3>
	...
	<table class="table table-hover myBooking">
		<thead>
			<tr>
				...
				<th>{{ Sitevariable::setVariables($data['language_val'],'eventus_135')}}</th>
		...
		<tbody>
		@foreach($data['my_booking'] as $booking)
		<tr>
			...
			<td><p>{{ Sitevariable::setVariables($data['language_val'],'eventus_18')}}: {{dateFormat($booking->check_in)}}</p> <p>{{ Sitevariable::setVariables($data['language_val'],'eventus_19')}}: {{dateFormat($booking->check_out)}}</p></td>
	...
	@else
	<p>{{ Sitevariable::setVariables($data['language_val'],'eventus_109')}}.</p>
	@endif
