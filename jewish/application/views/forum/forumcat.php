			...
			var fcomment_email = $("#fcomment_email").val();
  			if(IsEmail(fcomment_email)){
  			  $.ajax({
				  ...
				  {
	             $(".loader").hide();
			      alert('Your email id / username is mismatched');	 
                     return false;
	  ...
	  ?>
        ...
        <div style="display:none" class="commentrespond comrdesc comrply<?php echo $i; ?>">
            <form action="" id="replysubmit<?php echo $i;?>"  method="post">
                ...
                --><input id="email" name="reply_email"  class="reply_email" type="hidden" value="<?php echo $sesval['comuser_email'];  ?>" size="30" aria-required="true">
                </p>
...
<div class="firstsection">
   <?php if(!$sesval['comuser_email']){ ?>
    <p class="comment-form-author">
        <label for="author">Username <span class="required">*</span></label><br /> </p><?php }?>
        <input id="fcomment_name" required="required" name="fcomment_name" <?php if($sesval['comuser_email']){ echo 'type="hidden"';}else{?> type="text"<?php }?> value="<?php echo $sesval['comuser_name'];  ?>" size="30" aria-required="true">
   
  <?php if(!$sesval['comuser_email']){ ?>     
    <p class="comment-form-email">
        <label for="email">Email <span class="required">*</span></label> <br /> </p><?php }?>
        <input id="fcomment_email" name="fcomment_email"  <?php if($sesval['comuser_email']){ echo 'type="hidden"';}else{?> type="text"<?php }?> value="<?php echo $sesval['comuser_email'];  ?>" size="30" aria-required="true">
         <?php if(!$sesval['comuser_email']){ ?> <span style="padding-left:5px;font-size:14px; position: relative;top: -33px;">
        <span style="padding-right:5px; font-weight:bold;font-size:17px; ">OR </span> <a style="font-weight:bold; color:#57e2ca" href="<?php echo JEWISH_URL;?>/login/redirect/?redirect=<?php echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']?>"">Login</a> to your account and reply on the topic</span><?php }?>
     
   <input type="hidden" name="forum_id" value="<?php echo $forumdata['forum_id']; ?>" /> 
   <?php if(!$sesval['comuser_email']){?> <div class="checking">Go</div><?php } ?>
   </div>
