...
$session_data = $this->session->userdata('logged_in');
$count = 1;
$im = $this->db->query("SELECT * FROM simple_map  WHERE latitude!='' AND longitude!='' AND user_id='" . $frnd_id . "'");
$k = $im->result_array();
foreach ($im->result_array() as $row) {?>
<span class="innerw">
...
<?php
$count = 1;
$im = $this->db->query("SELECT DISTINCT `name`,`parent` FROM `multimap` WHERE `parent`!=0 AND user_id='" . $frnd_id . "'");
$k = $im->result_array();
foreach ($im->result_array() as $row) {?>
<span class="innerw">
...
<?php
$count = 1;
$im = $this->db->query("SELECT * FROM `waypoints` WHERE user_id='" . $frnd_id . "' group by trip_name");
foreach ($im->result_array() as $row) {
