...
$(document).ready(function() {
    $(document).on('click','.mark_this',function(){
		...
		$('#tab_'+$(this).data('markno')).addClass('active_this'); //alert($(this).data('markno'));
		/*$('#m >#m2 >.images').attr('data-marker',$(this).data('markno'));*/
		$('#m >#m2 >.checkboxFive > .images').val($(this).data('markno'));
		
		
		$('#m >#m2 >.checkboxFive >:checked').each(function() {
       $(this).removeClass();
     });
		//$('#m >#m2 >.images').removeClass();
		$('#m >#m2 >.checkboxFive >.images').addClass('im_'+$(this).data('markno'));
	...
	$('#m >#m2 >.checkboxFive >input[type=checkbox]').on('click',function(pro){ 
			if($(this).val()==''){
				...
				if ($(this).is(':checked')) {
					...
					$('#m >#m2 >.checkboxFive >.show_'+$(this).data('id')).html($(this).val()).show(); 
					$(this).prop('checked',true);
				}else{
					$(this).prop('checked',false);
					$('#m >#m2 >.checkboxFive >.show_'+$(this).data('id')).empty().hide();
				}
			}
		});
	...
	$('#m >#m2 >.checkboxFive >input[type=checkbox]').click(function(){   
			  var param=[];
...
<img src="<?php echo base_url().'upload/'.$row['image_file'];?>" height="80" width="80"/><br />
	 <div class="checkboxFive">
          ...
          <input name="image" class="images" data-imageid="<?php echo $row['id'];?>" type="checkbox" value="" id="checkboxFiveInput_<?php echo $c;?>" data-marker="" data-id="<?php echo $c; ?>" data-image="<?php echo $row['image_file'];?>" style="display:none;">
...
.image_container{margin: 3px !important; padding: 3px !important; border: 1px solid #66F !important; width: 80px !important; position: relative; float: left;  }
.mark_this{background-color:white; color:red; /*border:1px solid yellow;*/ margin:2px; cursor: pointer; }
.image_tabs{width: 330px !important; overflow-y: auto; }
.setpoint{display:none; padding: 10px; background-color: #69F; padding: 10px; cursor:pointer; color:white; margin-left: 14px; }
.active_this{background-color:blue; color:white; border:1px solid yellow; margin:2px; cursor: pointer; }
.numbering{opacity: 0.7; padding: 10px 15px 10px 15px; border-radius: 25px 25px 25px 25px; background-color: #69F; position: absolute; margin-top: -66px; color: white; font-size: 19px; font-weight: bold; margin-left: 20px; }
...
#target {width: 345px; }
.mrkt{border: 0; background-color: #69F; color: white; padding: 10px; }
.image_container > input[type=checkbox]{position: absolute; margin-top: -20px; margin-left: 5px; }
.labels{background-color:white; padding:20px; width:10px; }
.addrl{width:150px; }
.hedaddr{font-weight: bold; }
.check_mark{display:none; }
 #latitude2,#image_title,#longitude2,#start_point,#end_point2,#trip_name{background-color: #fff; font-family: Roboto; font-size: 15px; font-weight: 300; margin-left: 12px; padding: 0 11px 0 13px; text-overflow: ellipsis; width: 264px; height: 25px; }
  ...
  #trash { width: 96%; height: 10em; padding: 1%; margin-top: 40px; overflow-x: auto; }
  #trash h4 { line-height: 16px; margin: 0 0 0.4em; }
  #trash h4 .ui-icon { float: left; }
  #trash .gallery h5 { display: none; }
.images{z-index:10000; }
#target {width: 345px; }
.image_container > input[type=checkbox]{position: absolute; margin-top: -20px; margin-left: 5px; }
...
.image_tabs{ height: inherit; max-height: 250px; overflow-y: auto; margin-right: 2%;}
.image_tabs::-webkit-scrollbar {width: 12px; }
.image_tabs::-webkit-scrollbar-track {-webkit-box-shadow: inset 0 0 6px rgba(0,0,0,0.3); border-radius: 10px; }
.image_tabs::-webkit-scrollbar-thumb {border-radius: 10px; -webkit-box-shadow: inset 0 0 6px rgba(0,0,0,0.5); }
/*=========================*/
