...
FROM    multimap
        JOIN ( SELECT   MAX(id) AS id
               ...
               GROUP BY latitude
               HAVING   COUNT(*) >= 1
             ) parent ON parent.id = multimap.id WHERE parent='" . $this->uri->segment(4) . "'");
$k = $im->result_array();
...
foreach ($im->result_array() as $row) {
	$name = $row['name'];
	$ir = $this->db->query("SELECT * FROM `multimap` WHERE `latitude`='" . $row['latitude'] . "' AND `longitude`='" . $row['longitude'] . "' AND parent='" . $this->uri->segment(4) . "'");
