...
<?php
$im = $this->db->query("SELECT * FROM `waypoints` WHERE parent='" . $this->uri->segment(4) . "' LIMIT 0,1");
...
?>
      <?php
$count = 0;
$im = $this->db->query("SELECT DISTINCT  parent FROM `waypoints` WHERE parent='" . $this->uri->segment(4) . "'");
foreach ($im->result_array() as $row) {
...
var neighborhoods = [
<?php
$im = $this->db->query("SELECT * FROM `waypoints` WHERE parent='" . $this->uri->segment(4) . "'");
$k = $im->result_array();
foreach ($im->result_array() as $row) {?>
